...
function checkLocalStatus() {
    ...
    if (localStatus.length > 0) {
        ...
        index_1.handleError('please commit locally modified files or checkout first');
    }
}
...
function pullRemoteBranch(branchName) {
    ...
    if (shelljs_1.default.exec("git pull origin " + branchName).stderr) {
        index_1.handleError('git pull error');
    }
}
