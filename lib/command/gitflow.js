...
function finish(type, name) {
    return tslib_1.__awaiter(this, void 0, void 0, function () {
        var tagName, gitflow_1, localBranches, choices, answers;
        return tslib_1.__generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    git_1.checkLocalStatus();
                    ...
                    switch (type) {
                        case 'feature':
                            git_1.pullRemoteBranch('develop');
                            if (shelljs_1.default.exec("git merge --no-ff feature/" + name).stderr) {
                        ...
                        case 'hotfix':
                            git_1.pullRemoteBranch('master');
                            if (shelljs_1.default.exec("git merge --no-ff hotfix/" + name).stderr) {
                        ...
                        case 'release':
                            git_1.pullRemoteBranch('master');
                            if (shelljs_1.default.exec("git merge --no-ff release/" + name).stderr) {
                ...
                case 2:
                    ...
                    choices = localBranches.filter(function (branch) { return new RegExp("^" + gitflow_1.type + "/").test(branch); });
