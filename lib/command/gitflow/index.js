...
function default_1(program, gitflowConfig) {
    ...
    program
        ...
        switch (mode) {
            ...
            case 'start':
                if (gitFlowType) {
                    main_1.start(config.upstream, config.perfix, gitFlowType, options[gitFlowType]);
                }
                else {
                    main_1.start(config.upstream, config.perfix);
                }
                break;
            case 'finish':
                if (gitFlowType) {
                    main_1.finish(config.upstream, config.perfix, gitFlowType, options[gitFlowType]);
                }
                else {
                    main_1.finish(config.upstream, config.perfix);
                }
                break;
            default:
                main_1.gitflow(mode, config.upstream, config.perfix);
                break;
        }
    });
}
