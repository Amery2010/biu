...
function default_1(program, gitflowConfig) {
    ...
    program
        ...
        switch (mode) {
            ...
            case 'start':
                if (gitFlowType) {
                    main_1.start(config.prefix, gitFlowType, options[gitFlowType]);
                }
                else {
                    main_1.start(config.prefix);
                }
                break;
            case 'finish':
                if (gitFlowType) {
                    main_1.finish(config.prefix, gitFlowType, options[gitFlowType]);
                }
                else {
                    main_1.finish(config.prefix);
                }
                break;
            default:
                main_1.gitflow(mode, config.prefix);
                break;
        }
    });
}
